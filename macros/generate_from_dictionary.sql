...
{% macro generate_external_from_dictionary(table_only='false') %}
    ...
    {% for tbl in tables %}
        ...
        {% set query %}
            select  source_column_name, source_column_type, lower(stage_column_type) stage_column_type, column_order 
    ...
    {% set file_pattern = var('dictionary_file_pattern') %}
        ...
        {% if table_only == 'false' %}
            ...
            {% do sources_yaml.append(' LOCATION = @bde.raw.' ~ external_stage ) %}
