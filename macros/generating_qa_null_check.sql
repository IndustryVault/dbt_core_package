...
{% macro check_all_tables_null(schema_name) %}
   ...
   {% set template %}
       Select '{@table_name}' as table_name, count(*) row_count, count({@column_name}) not_null_count, count(*) - count({@column_name}) null_count, max(as_of_dt) max_date, min(as_of_dt) as min_date, count from bkmrtg.{@table_name}
   {% endset %}
